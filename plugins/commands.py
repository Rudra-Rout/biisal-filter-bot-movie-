...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/frymaxx</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/frymaxx').json()
...
async def send_movie_group(client, message):
    if MOVIE_GROUP_LINK:
        await message.reply(f"🎬 **Movie Group Link**: {MOVIE_GROUP_LINK}")
    else:
        await message.reply("🚫 Movie group link is not set. Please configure it in `info.py`.")
